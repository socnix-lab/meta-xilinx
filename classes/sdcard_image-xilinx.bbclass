...
IMAGE_CMD_xilinx-sdimg () {
	...
	sdroot_dtb=sdroot-${MACHINE}.dtb
	gen_sdroot_dtb $sdroot_dtb

	its_fn="sdroot-${KERNEL_IMAGETYPE}-its-${PV}-${PR}-${MACHINE}.its"
	${XILINXBASE}/scripts/bin/mkits.sh -v "${MACHINE}" \
		...
		-d "${sdroot_dtb}" -c 1 \
		-o ${its_fn}
	uboot-mkimage -f ${its_fn} fitImage

	# copy fit.itb to boot.img
	mcopy -i ${WORKDIR}/boot.img -s ${WORKDIR}/fitImage ::${SDIMG_KERNELIMAGE}

	its_symlink_name=sdroot-${KERNEL_IMAGETYPE}-its-${MACHINE}.itx
	install -m 0644 ${its_fn} ${DEPLOY_DIR_IMAGE}
	ln -sf ${its_fn} ${its_symlink_name}
...
EOF
		...
		dtc -I dts -O dtb -o ${1} ${sdroot_dts}
		install -m 0644 ${1} ${DEPLOY_DIR_IMAGE}
	else
		install -m 0644 $target_dtb ${1}
	fi
}
